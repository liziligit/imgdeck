...
- 支持矩形马赛克：先在整张 A4 预览上拖出区域，再点击“应用马赛克”；块大小默认 15、可设为 2～100，导出结果同步保留马赛克
...
8. 点击“矩形马赛克”后，在整张 A4 预览上拖出矩形区域；“应用马赛克”会在画出区域前保持灰色，画出后可点击应用。再次点击高亮的“矩形马赛克”可取消尚未应用的框线，不影响已应用区域。块大小默认 15、滑动范围为 2～100，马赛克可用 `⌘Z`/`⇧⌘Z`撤销或重做。
9. 根据需要选择“保持长宽比”或“自由拉伸”。可点击“重置当前图片”恢复默认，使用 `⌘Z`/`⇧⌘Z`撤销或重做。
10. 设置分辨率与单位，点击“预览”。
11. 选择 PNG 或 JPG，点击“保存图片”。
